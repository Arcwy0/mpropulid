...
# Load the JSON configuration once
photostyles_data = load_photostyles()

def process_one_image(image_path, photostyle):
    #TODO: add config.json for parameters for different photo styles
    try:
    ...
    if not style_params:
        raise ValueError(f"Photo style '{photostyle}' not found in configuration.")
    processed_image, seed, _ = flux_generator.generate_image(
...
process_one_image(image_path='example_inputs/rihanna.webp', photostyle = 'Closeup')